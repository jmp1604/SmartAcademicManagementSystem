 ...
 * Matches PHP dashboard functionality
 * ============================================
 */
...
async function loadAdminProfile() {
    try {
        ...
        document.getElementById('adminRole').textContent = userData.adminLevel === 'super_admin' 
            ? 'Super Admin' 
            : 'Admin';
            ...
            <div class="lab-card" id="lab-${lab.lab_id}" onclick="toggleLabSchedules('${lab.lab_id}')">
                ...
                <div class="current-session-info">
                    ...
                    <div class="current-session-detail">
                        ...
                        <strong>${lab.currentSession.lab_schedules.subjects.subject_code}</strong> - ${lab.currentSession.lab_schedules.subjects.subject_name}
                    </div>
                    <div class="current-session-detail">
                        <i class="fa-solid fa-chalkboard-teacher"></i>
                        ${lab.currentSession.lab_schedules.professors.first_name} ${lab.currentSession.lab_schedules.professors.last_name}
                    </div>
                    <div class="current-session-detail">
                    ...
                    <div class="current-session-detail">
                        ...
                        ${formatTime(lab.currentSession.lab_schedules.start_time)} - ${formatTime(lab.currentSession.lab_schedules.end_time)}
                    </div>
                </div>
                ${lab.isDismissing ? `
                ...
                <button class="schedules-toggle" onclick="event.stopPropagation(); toggleLabSchedules('${lab.lab_id}')">
                    <i class="fa-solid fa-chevron-down"></i>
                    <span>View All Schedules</span>
                </button>
                
                <div class="schedules-container" id="schedules-${lab.lab_id}">
                    <div class="schedules-content" id="schedules-content-${lab.lab_id}">
...
async function loadLabSchedules(labId, container) {
    try {
        ...
        const schedulesWithEnrollment = await Promise.all(schedules.map(async (schedule) => {
            ...
            const isActiveNow = schedule.day_of_week === currentDay 
        ...
        displayLabSchedules(container, schedulesWithEnrollment);
        
    } catch (error) {
        console.error('Schedules error:', error);
        container.innerHTML = '<p class="no-schedules">Failed to load schedules.</p>';
    }
}
...
function displayLabSchedules(container, schedules) {
    ...
    days.forEach(day => {
        if (grouped[day].length > 0) {
            ...
            grouped[day].forEach(schedule => {
                const professorName = `${schedule.professors.first_name} ${schedule.professors.middle_name || ''} ${schedule.professors.last_name}`.trim();
                const time = `${formatTime(schedule.start_time)} - ${formatTime(schedule.end_time)}`;
                
                html += `
                    <div class="schedule-item ${schedule.isActiveNow ? 'active-now' : ''}">
                        <div class="schedule-time">
                            ${time}
                            ${schedule.isActiveNow ? '<span class="active-now-badge">● LIVE</span>' : ''}
                        </div>
                        <div class="schedule-subject">${schedule.subjects.subject_code} - ${schedule.subjects.subject_name}</div>
                        <div class="schedule-detail">👨‍🏫 ${professorName} • Section: ${schedule.section}</div>
                        <div class="schedule-detail">🎓 ${schedule.semester} ${schedule.school_year} • ✅ ${schedule.enrolledCount} enrolled</div>
                    </div>
                `;
            });
...
function setSidebarActive(href) {
    try {
        const parentLinks = parent.document.querySelectorAll('.menu-item');
        parentLinks.forEach(link => link.classList.remove('active'));
        const targetLink = parent.document.querySelector(`.menu-item[href="${href}"]`);
        if (targetLink) targetLink.classList.add('active');
    } catch (error) {
        console.error('Cannot access parent:', error);
    }
}
